...
import org.bitman.ay27.request.RequestManager;
...
public class ImageCacheManager {
    // 取运行内存阈值的1/8作为图片缓存
    private static final int MEM_CACHE_SIZE = 1024 * 1024 * ((ActivityManager) PickerApplication.getContext()
                    ...
                    if (!isImmediate && defaultImageDrawable != null) {
                        TransitionDrawable transitionDrawable = new TransitionDrawable(
                                new Drawable[]{
                                        ...
                                        new BitmapDrawable(PickerApplication.getContext().getResources(),
                                                response.getBitmap())
                                }
                        );
