...
public class ChooseArea extends View {
    ...
    public ChooseArea(Context context) {
        super(context);
        BOUND = (int) (getContext().getResources().getDisplayMetrics().density * 25);
    }
    ...
    private void correctPoints() {
        ...
        if (!flag) {
            Toast.makeText(getContext(), "无法裁剪", Toast.LENGTH_SHORT).show();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final int circleR = (int) (16 * getContext().getResources().getDisplayMetrics().density);

        // 画出四个角的小圈
        for (int i = 0; i < 4; i++) {
            canvas.drawCircle(p[i].x, p[i].y, circleR, paintDrawCircle);
        }
    }
